...
SmartTrainer::SmartTrainer(vector<string> &words): RandomTrainer(words) {

}

string SmartTrainer::selectWords() {
    return selectWords(m_lexicon);
}

string SmartTrainer::selectWords(vector<string> &words) 
{
    // If m_words is not empty, make empty
    if (m_words.size() > 0){
        m_words.erase();
...
string SmartTrainer::getWords() {
    return m_words;
}

bool SmartTrainer::ended() {
    return m_end;
}

int SmartTrainer::getMistakeCount(int ch) {
    if (m_char_stats.count(ch)) // Check if m_char_stats contains charcter
        return m_char_stats[ch];
}

void SmartTrainer::setMistakeCount(int ch, int count) {
    m_char_stats[ch] = count;
}